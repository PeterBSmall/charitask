import 'package:flutter/material.dart';

import 'package:charitask/domain/organization/organization_size.dart';

import 'package:charitask/shared/design_system/journey/ct_journey_button.dart';
import 'package:charitask/shared/design_system/journey/ct_journey_constants.dart';
import 'package:charitask/shared/design_system/journey/ct_journey_controller.dart';
import 'package:charitask/shared/design_system/journey/ct_journey_header.dart';
import 'package:charitask/shared/design_system/journey/ct_journey_progress.dart';
import 'package:charitask/shared/design_system/journey/ct_journey_selection_card.dart';

import 'package:charitask/shared/data/organization/organization_sizes.dart';

class CTOrganizationSizeStep extends StatefulWidget {
  final CTJourneyController journeyController;
  final VoidCallback onContinue;
  final VoidCallback onBack;

  const CTOrganizationSizeStep({
    super.key,
    required this.journeyController,
    required this.onContinue,
    required this.onBack,
  });

  @override
  State<CTOrganizationSizeStep> createState() => _CTOrganizationSizeStepState();
}

class _CTOrganizationSizeStepState extends State<CTOrganizationSizeStep> {
  OrganizationSize? _selectedSize;

  @override
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const CTJourneyProgress(currentStep: 5, totalSteps: 7),

        const SizedBox(height: CTJourneySpacing.progressToHeader),

        const CTJourneyHeader(
          title: 'Hello Peter.',
          question: 'How many people are part of your organization?',
          subtitle: 'This helps ChariTask personalize your workspace.',
          icon: Icons.groups_outlined,
        ),

        const SizedBox(height: 28),

        Expanded(
          child: ListView.builder(
            itemCount: organizationSizes.length,
            itemBuilder: (context, index) {
              final option = organizationSizes[index];

              return CTJourneySelectionCard(
                icon: option.icon,
                title: option.title,
                subtitle: option.subtitle,
                selected: _selectedSize == option.size,
                onTap: () {
                  setState(() {
                    _selectedSize = option.size;
                  });

                  widget.journeyController.updateOrganizationSize(option.size);
                },
              );
            },
          ),
        ),

        const SizedBox(height: 20),

        Row(
          children: [
            Expanded(
              child: OutlinedButton(
                onPressed: widget.onBack,
                child: const Text('Back'),
              ),
            ),

            const SizedBox(width: 16),

            Expanded(
              child: CTJourneyButton(
                text: 'Continue',
                onPressed: _selectedSize == null
                    ? null
                    : () {
                        widget.journeyController.updateOrganizationSize(
                          _selectedSize!,
                        );

                        widget.onContinue();
                      },
              ),
            ),
          ],
        ),
      ],
    );
  }
}
